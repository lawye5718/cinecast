...
# ---------------------------------------------------------------------------
# P0: Control Character Preprocessing
# ---------------------------------------------------------------------------

class TestControlCharacterCleaning:
    def test_strips_null_bytes(self):
        """Null bytes and other low control chars should be replaced."""
        import re
        raw = '[\x00{"type": "narration", "speaker": "narrator", "gender": "male", "emotion": "平静", "content": "测试"}]'
    ...
    def test_preserves_newlines(self):
        """Newlines (\\n) should be kept since JSON uses them for formatting."""
        import re
        raw = '[\n{"type": "narration", "speaker": "narrator", "gender": "male", "emotion": "平静", "content": "你好"}\n]'
        cleaned = re.sub(r'[\x00-\x09\x0b\x0c\x0e-\x1f]', ' ', raw)
        result = json.loads(cleaned)
        assert len(result) == 1

    def test_strips_tabs_and_vertical_tabs(self):
        """Tabs (\\t=0x09) and vertical tabs (\\x0b) should be replaced."""
        import re
        raw = '[{"type":\t"narration",\x0b"speaker": "narrator", "gender": "male", "emotion": "平静", "content": "OK"}]'
        cleaned = re.sub(r'[\x00-\x09\x0b\x0c\x0e-\x1f]', ' ', raw)
        result = json.loads(cleaned)
        assert result[0]["type"] == "narration"
...
# ---------------------------------------------------------------------------
# P2: Audacity Export Wiring
# ---------------------------------------------------------------------------

class TestAudacityExportWiring:
    def test_phase3_calls_export_audacity(self):
        """Verify that phase_3_cinematic_mix calls export_audacity."""
...
# ---------------------------------------------------------------------------
# P1: Emotion Parameter Passing
# ---------------------------------------------------------------------------

class TestEmotionParameterPassing:
    def test_phase2_passes_emotion(self):
        """Verify that phase_2_render_dry_audio passes emotion to render_dry_chunk."""
        main_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            "main_producer.py",
        )
