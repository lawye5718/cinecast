...
- Default fallback 'eric' is used when neither 'voice' nor 'speaker' is set
...
class TestPresetVoiceMapping:
    ...
    def test_voice_get_with_speaker_fallback(self):
        """Preset mode should use voice_cfg.get('voice', voice_cfg.get('speaker', 'eric'))."""
        body = _get_render_func_body()
        assert 'voice_cfg.get("voice", voice_cfg.get("speaker", "eric"))' in body
    ...
    def test_default_eric_fallback(self):
        """Default voice fallback 'eric' must be present for safety."""
        body = _get_render_func_body()
        assert '"eric"' in body
...
class TestCloneVoiceDefense:
    ...
    def test_clone_voice_fallback(self):
        """Clone mode voice fallback should also use eric as default."""
        ...
        assert 'voice_cfg.get("voice", voice_cfg.get("speaker", "eric"))' in clone_block
