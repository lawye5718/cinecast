#!/usr/bin/env python3
"""
Alexandria项目修复脚本 - 应用CineCast中验证的实现
修复WAV生成问题和本地大模型串行处理
"""
...
"""
    ...
    def _predict_gender(self, speaker_name: str) -> str:
        """简单性别预测"""
        female_indicators = ['女士', '小姐', '夫人', '妈妈', '姐姐', '妹妹', '女儿', '阿姨', '奶奶']
        male_indicators = ['先生', '少爷', '老爷', '爸爸', '哥哥', '弟弟', '儿子', '叔叔', '爷爷']
...
def update_config_for_local_models():
    ...
    config["tts"] = {
        "mode": "local",  # 或 "mlx" 如果支持
        "device": "auto",
        "language": "Chinese",
        "compile_codec": False,
        "model_path": "./models/Qwen3-TTS-MLX-0.6B"  # 使用cinecast中验证的模型路径
    }
