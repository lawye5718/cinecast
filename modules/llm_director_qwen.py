#!/usr/bin/env python3
"""
CineCast Qwen-Flash 大模型剧本预处理器
阶段一：剧本化与微切片 (Script & Micro-chunking)
实现宏观剧本解析 -> 自动展开为微切片剧本
使用阿里云百炼Qwen-Flash模型替代GLM API
"""
...
class QwenScriptDirector:
    ...
    def __init__(self, api_key=None, global_cast=None, cast_db_path=None, **kwargs):
        if kwargs:
            logger.warning(f"⚠️ QwenScriptDirector 收到未识别的参数（已忽略）: {list(kwargs.keys())}")
        
        # 初始化Qwen客户端
        self.api_key = api_key or os.environ.get("DASHSCOPE_API_KEY", "")
        if not self.api_key:
            logger.warning("⚠️ 未设置 DASHSCOPE_API_KEY，智能配音模式将无法使用 Qwen-Flash API。")
    ...
    def _test_api_connection(self):
        ...
        try:
            ...
            if completion.choices and len(completion.choices) > 0:
                logger.info("✅ Qwen-Flash API 服务连接正常")
                return True
            else:
                logger.warning("❌ Qwen-Flash API 服务响应异常")
                return False
        except Exception as e:
            logger.warning(f"❌ 无法连接到 Qwen-Flash API 服务: {e}")
            return False
    
    def _chunk_text_for_llm(self, text: str, max_length: int = 997000) -> List[str]:
        """🌟 防止章节过长，按段落切分为安全大小给 LLM 处理
        Qwen-Flash支持1M上下文，最大输入997k字符"""
    ...
    def parse_and_micro_chunk(self, text: str, chapter_prefix: str = "chunk", max_length: int = 997000) -> List[Dict]:
        ...
        Args:
            ...
            max_length: LLM 单次处理的最大字符数上限，默认997000（接近1M上限）
        """
        # 第一步：生成宏观剧本
    ...
    def parse_text_to_script(self, text: str, max_length: int = 997000) -> List[Dict]:
        ...
        max_length 设为 997000 字符（接近1M上限），为输出 token 预算留足安全余量。

        Args:
            text: 待处理的章节文本
            max_length: LLM 单次处理的最大字符数上限，默认997000
        """
        ...
        for i, chunk in enumerate(text_chunks):
            ...
            # Qwen API 的频率限制由 _request_llm 内部的退避逻辑自动控制，无需人为节流
    ...
    def generate_chapter_recap(self, prev_chapter_text: str) -> str:
        ...
        try:
            completion = self.client.chat.completions.create(
                ...
                temperature=0.5,
                top_p=0.8,
                max_tokens=32768,  # Qwen-Flash最大输出32k tokens
            )
...
"""
        ...
        # 🌟 Qwen-Flash API 使用 1M 上下文窗口，最大输出 32k tokens
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(f"⚠️ Qwen-Flash API 请求异常 (尝试 {attempt + 1}/{max_retries}): {e}")
                if attempt < max_retries - 1:
                    ...
                    raise RuntimeError(f"❌ 超过最大重试次数，Qwen-Flash API 请求彻底失败: {e}")

        raise RuntimeError("❌ 超过最大重试次数，Qwen-Flash API 请求彻底失败。请检查您的API密钥和网络连接。")
